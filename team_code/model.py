    ...
    def __init__(self, config):
        ...
        if self.config.use_semantic:
            self.semantic_decoder = t_u.PerspectiveDecoder(
                ...
                scale_factor_0=self.backbone.perspective_upsample_factor
                // self.config.deconv_scale_factor_0,
                scale_factor_1=self.backbone.perspective_upsample_factor
                // self.config.deconv_scale_factor_1,
            )
            ...
            if self.config.transformer_decoder_join:
                ...
                if self.config.tp_attention:
                    self.tp_encoder = nn.Sequential(
                        nn.Linear(target_point_size, 128),
                        nn.ReLU(inplace=True),
                        nn.Linear(128, self.config.gru_input_size),
                    )
                    self.tp_pos_embed = nn.Parameter(
                    ...
                    decoder_layer = TransformerDecoderLayerWithAttention(
                        self.config.gru_input_size,
                        self.config.num_decoder_heads,
                        activation=nn.GELU(),
                    )
                    self.join = TransformerDecoderWithAttention(
                        decoder_layer,
                ...
                if self.config.use_wp_gru:
                    if self.config.multi_wp_output:
                        self.wp_query = nn.Parameter(
                            torch.zeros(
                                1,
                                2 * (self.config.pred_len // self.config.wp_dilation)
                                + 1,
                                self.config.gru_input_size,
                            )
                        )

                        self.wp_decoder = GRUWaypointsPredictorInterFuser(
                        ...
                        self.wp_decoder_1 = GRUWaypointsPredictorInterFuser(
                            input_dim=self.config.gru_input_size,
                            hidden_size=self.config.gru_hidden_size,
                            waypoints=(config.pred_len // self.config.wp_dilation),
                            target_point_size=target_point_size,
                ...
                if self.config.use_wp_gru:
                    self.wp_decoder = GRUWaypointsPredictorTransFuser(
                        self.config,
                ...
                if self.config.use_controller_input_prediction:
                    self.checkpoint_decoder = GRUWaypointsPredictorTransFuser(
                        self.config,
        ...
        self.loss_bev_semantic = nn.CrossEntropyLoss(
            weight=self.bev_semantic_weights,
            ...
            if self.config.transformer_decoder_join:
                ...
                if self.config.use_controller_input_prediction:
                    if self.config.tp_attention:
                        ...
                        fused_features = torch.cat(
                            (fused_features, tp_token.unsqueeze(1)), axis=1
                        )
                        joined_checkpoint_features, attention = self.join(
                        ...
                        if self.extra_sensors:
                            add = 1
                            speed_attention = gru_attention[
                                num_pixel_tokens : num_pixel_tokens + add
                            ]
                        tp_attention = gru_attention[num_pixel_tokens + add :]
                        attention_weights = [
                            vision_attention.item(),
    ...
    def compute_loss(
        ...
        if self.config.use_wp_gru:
            if self.config.multi_wp_output:
                ...
                loss_wp_1 = torch.mean(
                    torch.abs(pred_wp_1 - waypoint_label), dim=(1, 2)
                )
                stacked_wp_losses = torch.stack((loss_wp, loss_wp_1), dim=1)
                loss_wp_total, selection_labels = torch.min(
                    stacked_wp_losses, dim=1, keepdim=True
                )
        ...
        if self.config.use_bev_semantic:
            visible_bev_semantic_label = (
                self.valid_bev_pixels.squeeze(1).int() * bev_semantic_label
            )
            # Set 0 class to ignore index -1
            visible_bev_semantic_label = (
                self.valid_bev_pixels.squeeze(1).int() - 1
            ) + visible_bev_semantic_label
            loss_bev_semantic = self.loss_bev_semantic(
                pred_bev_semantic, visible_bev_semantic_label
            )
