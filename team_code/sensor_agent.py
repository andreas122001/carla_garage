...
class SensorAgent(autonomous_agent.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
        ...
        with open(
            os.path.join(self.config_path, "config.json"), "rt", encoding="utf-8"
        ) as f:
            json_config = f.read()
        ...
        self.config.brake_uncertainty_threshold = float(
            os.environ.get(
                "UNCERTAINTY_THRESHOLD", self.config.brake_uncertainty_threshold
            )
        )
        ...
        if int(os.environ.get("SLOWER", 0)):
            print(
                f"Reduce target speeds during evaluation by factor {self.config.slower_factor}."
            )
            self.inference_target_speeds = [
                self.config.slower_factor * speed for speed in self.config.target_speeds
            ]
        else:
        ...
        for file in os.listdir(self.config_path):
            if file.endswith(".pth") and file.startswith("model"):
                ...
                state_dict = torch.load(
                    os.path.join(self.config_path, file),
        ...
        self.state_log = deque(
            maxlen=max((self.config.lidar_seq_len * self.config.data_save_freq), 2)
        )

        # Temporal LiDAR
        self.lidar_buffer = deque(
            maxlen=self.config.lidar_seq_len * self.config.data_save_freq
        )
    ...
    def _init(self):
        ...
        try:
            ...
            earth_radius_equa = (
                6378137.0  # Constant from CARLA leaderboard GPS simulation
            )
    ...
    def tick(self, input_data):
        ...
        if self.config.backbone not in ("aim"):
            result["lidar"] = t_u.lidar_to_ego_coordinate(
        ...
        if not self.filter_initialized:
            ...
            self.ukf.x = np.array(
                [gps_pos[0], gps_pos[1], t_u.normalize_angle(compass), speed]
            )
        ...
        self.ukf.update(
            np.array([gps_pos[0], gps_pos[1], t_u.normalize_angle(compass), speed])
        )
        filtered_state = self.ukf.x
        self.state_log.append(filtered_state)
        result["gps"] = filtered_state[0:2]

        waypoint_route = self._route_planner.run_step(
            np.append(filtered_state[0:2], gps_pos[2])
        )
        ...
        ego_target_point = torch.from_numpy(ego_target_point[np.newaxis]).to(
            self.device, dtype=torch.float32
        )
        ...
        if self.config.two_tp_input:
            ego_target_point_next = t_u.inverse_conversion_2d(
                target_point_next[:2], result["gps"], result["compass"]
            )
            ego_target_point_next = torch.from_numpy(
                ego_target_point_next[np.newaxis]
            ).to(self.device, dtype=torch.float32)
            result["target_point_next"] = ego_target_point_next

        result["speed"] = torch.FloatTensor([speed]).to(
            self.device, dtype=torch.float32
        )

        if self.save_path is not None:
            pass
            waypoint_route = self._waypoint_planner.run_step(
                np.append(result["gps"], gps_pos[2])
            )
            waypoint_route = extrapolate_waypoint_route(
                waypoint_route, self.config.route_points
            )
            route = np.array([[node[0][0], node[0][1]] for node in waypoint_route])[
    ...
    def run_step(
        ...
        if self.config.backbone not in ("aim"):
            ...
            if len(self.lidar_buffer) < (
                self.config.lidar_seq_len * self.config.data_save_freq
            ):
                self.lidar_last = deepcopy(tick_data["lidar"])
        ...
        if self.config.backbone in ("aim"):  # Image only method
            ...
            lidar_bev = torch.zeros(
                (
                    1,
                    1 + int(self.config.use_ground_plane),
            ...
            for i in lidar_indices:
                ...
                lidar_histogram = (
                    torch.from_numpy(lidar_histogram)
                    .unsqueeze(0)
                    .to(self.device, dtype=torch.float32)
                )
        ...
        for i in range(self.model_count):
            if self.config.backbone in ("transFuser", "aim", "bev_encoder"):
                ...
                if self.config.detect_boxes and (
                    ...
                    or self.config.backbone in ("aim")
                    or self.stop_sign_controller
                ):
                    pred_bounding_box = self.nets[i].convert_features_to_bb_metric(
                        pred_bb_features
                    )
                else:
                ...
                raise ValueError(
                    "The chosen vision backbone does not exist. The options are: transFuser, aim, bev_encoder"
                )
        ...
        if self.config.detect_boxes and (
            ...
            bbs_vehicle_coordinate_system = t_u.non_maximum_suppression(
        ...
        if self.config.use_controller_input_prediction:
            pred_target_speed_ensemble = torch.stack(pred_target_speeds, dim=0).mean(
                dim=0
            )  # average across ensemble models' prediction
            ...
            if self.uncertainty_weight:
                ...
                if uncertainty[0] > self.config.brake_uncertainty_threshold:
                    ...
                    pred_target_speed_scalar = sum(
                        uncertainty * self.inference_target_speeds
                    )
            else:
                pred_target_speed_index = torch.argmax(pred_target_speed_ensemble)
                pred_target_speed_scalar = self.inference_target_speeds[
                    pred_target_speed_index
                ]
        ...
        if compute_debug_output:
            ...
            self.nets[0].visualize_model(
                ...
                pred_wp,
                target_point_next=(
                    tick_data["target_point_next"] if self.config.two_tp_input else None
                ),
        ...
        if (
            self.config.inference_direct_controller
            and self.config.use_controller_input_prediction
        ):
            pred_checkpoints = (
                torch.stack(pred_checkpoints, dim=0).mean(dim=0).detach().cpu().numpy()
            )
            steer, throttle, brake = self.nets[0].control_pid_direct(
                pred_checkpoints, pred_target_speed_scalar, gt_velocity
            )
        ...
        if self.force_move > 0:
            ...
            if self.config.backbone not in ("aim"):
                ...
                safety_box = safety_box[
                    safety_box[..., 2] > self.config.safety_box_z_min
                ]
                safety_box = safety_box[
                    safety_box[..., 2] < self.config.safety_box_z_max
                ]

                # y-axis
                safety_box = safety_box[
                    safety_box[..., 1] > self.config.safety_box_y_min
                ]
                safety_box = safety_box[
                    safety_box[..., 1] < self.config.safety_box_y_max
                ]

                # x-axis
                safety_box = safety_box[
                    safety_box[..., 0] > self.config.safety_box_x_min
                ]
                safety_box = safety_box[
                    safety_box[..., 0] < self.config.safety_box_x_max
                ]
    ...
    def stop_sign_controller_step(self, ego_speed):
        ...
        extent = carla.Vector3D(
            self.config.ego_extent_x, self.config.ego_extent_y, self.config.ego_extent_z
        )
        ...
        if len(self.stop_sign_buffer) > 0:
            ...
            if (
                t_u.check_obb_intersection(stop_carla_box, car_box)
                and self.clear_stop_sign <= 0
            ):
                if ego_speed > 0.01:
    ...
    def bb_detected_in_front_of_vehicle(self, ego_speed):
        ...
        # Safety box
        bremsweg = (
            (ego_speed.cpu().numpy().item() * 3.6) / 10.0
        ) ** 2 / 2.0  # Bremsweg formula for emergency break
        safety_x = np.clip(
            bremsweg + 1.0, a_min=2.0, a_max=4.0
        )  # plus one meter is the car.
    ...
    def align_lidar(
        self, lidar, x, y, orientation, x_target, y_target, orientation_target
    ):
    ...
    def update_stop_box(
        self, boxes, x, y, orientation, x_target, y_target, orientation_target
    ):
        ...
        if self.save_path is not None:
            ...
            if self.config.tp_attention:
                if len(self.tp_attention_buffer) > 0:
                    print(
                        "Average TP attention: ",
                        sum(self.tp_attention_buffer) / len(self.tp_attention_buffer),
                    )
                    with gzip.open(
                        self.save_path / "tp_attention.json.gz", "wt", encoding="utf-8"
                    ) as f:
...
class EgoModel:
    ...
    def forward(self, locs, yaws, spds, acts):
        ...
        beta = math.atan(
            self.rear_wb / (self.front_wb + self.rear_wb) * math.tan(wheel)
        )
        yaws = yaws.item()
        spds = spds.item()
        ...
        if self.ego_vehicle_model:
            if brake:
                spds = spds * 3.6
                features = np.array(
                    [spds, spds**2, spds**3, spds**4, spds**5, spds**6, spds**7]
                ).T

                next_spds = (features @ self.brake_values).item() / 3.6
            else:
                throttle = np.clip(throt, 0.0, 1.0)
