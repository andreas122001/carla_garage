...
def make_bash(data_save_root, code_dir, route_file_number, agent_name, route_file, ckeckpoint_endpoint, save_pth, seed):
    jobfile = f"{code_dir}/{data_save_root}/start_files/{route_file_number}.sh"
    # create folder
...
sh {carla_root}/CarlaUE4.sh --world-port=$FREE_WORLD_PORT -RenderOffScreen -nosound -graphicsadapter=0 -carla-streaming-port=$FREE_STREAMING_PORT &

sleep 6  # wait for Carla to start

nvidia-smi
echo "Using python: $(which python)"
echo "Using Carla API version: $(pip freeze | grep carla)"
...
def make_jobsub_file(data_save_root, code_dir, jobname, route_file_number, singularity_img, \
                     partition_name, timeout="3-00:00:00"):
    os.makedirs(f"{code_dir}/{data_save_root}/slurm/run_files/logs", exist_ok=True)
    os.makedirs(f"{code_dir}/{data_save_root}/slurm/run_files/job_files", exist_ok=True)
    jobfile = f"{code_dir}/{data_save_root}/slurm/run_files/job_files/{route_file_number}.sh"
    qsub_template = f"""#!/bin/bash
#SBATCH --job-name={jobname}_{route_file_number}
#SBATCH --account=share-ie-idi
#SBATCH --partition={partition_name}
# #SBATCH --nodelist=idun-07-05  # TODO: THIS IS TEMPORARY
#SBATCH -o {code_dir}/{data_save_root}/slurm/run_files/logs/out_{route_file_number}.log
#SBATCH -e {code_dir}/{data_save_root}/slurm/run_files/logs/err_{route_file_number}.log
#SBATCH --nodes=1
...
pwd
echo "start script"
sh {code_dir}/{data_save_root}/start_files/{route_file_number}.sh $FREE_STREAMING_PORT $FREE_WORLD_PORT $TM_PORT
"""
...
if __name__ == "__main__":
    max_num_parallel_jobs = 25
    partition = "GPUQ"
    job_name = "expert-pdml-smol"
    username = "andrebw"
    code_root = "/cluster/work/andrebw/carla_garage"
    date = datetime.today().strftime("%Y_%m_%d")
    singularity_image = "path/to/carla_img.sif"
    dataset_name = "smol_pdml_town13_selection"# + date
    root_folder = f"database/"  # With ending slash
    data_save_directory = root_folder + dataset_name
    log_root = f"{data_save_directory}/slurm"

    # route_folder = "/cluster/work/andrebw/carla_garage/leaderboard/data_validation"
    # route_folder = f"{code_root}/data/50x36_town_13"
    route_folder = f"{code_root}/data/town13_selection"
    # route_folder = f"{code_root}/data/50x38_town_12"
    # route_folder = f"{code_root}/data/50x38_town_12/Accident"
    # route_folder = f"{code_root}/data"
    print(route_folder)
    ...
    for repetition in range(repetitions):
        for route in routes:
            ...
            routefile_number = f'{route.split("/")[-1].split(".")[0]}_{repetition}'  # this is the number in the xml file name, e.g. 22_0.xml
            ckpt_endpoint = f"{code_root}/{data_save_directory}/results/{scenario_type}/{routefile_number}_result.json"

            save_path = f"{code_root}/{data_save_directory}/data/{scenario_type}/Routes_{scenario_type}_Repetition{repetition}/"
            Path(save_path).mkdir(parents=True, exist_ok=True)
            # agent = "/cluster/work/andrebw/agents/team_code/data_agent.py"
            # agent = f"{code_root}/../carla_garage_main/team_code/data_agent.py"
            ...
            if os.path.exists(f"{log_root}/run_files/logs/qsub_err{routefile_number}.log"):
                ...
                os.system(f"cp {log_root}/run_files/logs/qsub_err{routefile_number}.log {log_root}/run_files/logs_{routefile_number}_{time_now_log}")
                os.system(f"cp {log_root}/run_files/logs/qsub_out{routefile_number}.log {log_root}/run_files/logs_{routefile_number}_{time_now_log}")
